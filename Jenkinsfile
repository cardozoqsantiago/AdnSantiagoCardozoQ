    ...
    stage('Checkout') {
      steps{
        ...
        checkout([
            ...
            submoduleCfg: [], 
            userRemoteConfigs: [[
                credentialsId: 'GitHub_cardozoqsantiago', 
                url:'https://github.com/cardozoqsantiago/AdnSantiagoCardozoQ'
            ]]
            ])
...
post {
    ...
    failure {
      echo 'This will run only if failed'
	    mail (to: 'santiago.cardozo@ceiba.com.co',subject: "Failed Pipeline:${currentBuild.fullDisplayName}",body: "Something is wrong with ${env.BUILD_URL}")
    }
    unstable {
